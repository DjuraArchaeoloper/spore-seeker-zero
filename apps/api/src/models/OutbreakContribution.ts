...
export const OUTBREAK_EVENT_TYPES = [
  "direct_birth",
...
const outbreakContributionSchema = new Schema<OutbreakContribution>(
  ...
  {
    versionKey: false,
    collection: "outbreak_contributions",
  },
);

outbreakContributionSchema.index(
  { eventType: 1, birthEventKey: 1 },
  { unique: true },
);
outbreakContributionSchema.index(
...
export const OutbreakContributionModel =
  (mongoose.models.OutbreakContribution as
    | Model<OutbreakContribution>
    | undefined) ??
  mongoose.model<OutbreakContribution>(
    "OutbreakContribution",
    outbreakContributionSchema,
  );
